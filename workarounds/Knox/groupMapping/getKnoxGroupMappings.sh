#!/usr/bin/env bash
#===============================================================================
# Script: getKnoxGroupMappings.sh
# Purpose: Extract Cloudera Manager cluster configuration data via API
...
#===============================================================================
# Function: extract_cluster_configuration - Extract cluster configuration data
#===============================================================================
extract_cluster_configuration() {
    log_message "INFO" "Extracting cluster configuration data..."
    
    local api_url="${CM_SERVER}/api/${CM_API_VERSION}/clusters/${CM_CLUSTER_NAME}/services?view=EXPORT"
    local temp_file="/tmp/cm_config_extract_$$"
    
    # Extract configuration data using jq for structured output
    if curl -s -L -k -u "${WORKLOAD_USER}:${WORKLOAD_USER_PASS}" -X GET "$api_url" > "$temp_file"; then
        if jq -r '[
            # Service configurations
            ( .items[] | { sn: .name, sdn: .displayName, st: .type, c: .config.items[] } | 
              {serviceName: .sn, serviceDisplayName: .sdn, serviceType: .st, name: .c.name, value: .c.value} ),
            
            # Role configurations
            ( .items[] | { sn: .name, sdn: .displayName, st: .type, r: .roles[] } | 
              {sn: .sn, sdn: .sdn, st: .st, rn: .r.name, rt: .r.type, rc: .r.config.items[], rcgn: .r.roleConfigGroupRef.roleConfigGroupName} | 
              {serviceName: .sn, serviceDisplayName: .sdn, serviceType: .st, roleName: .rn, roleType: .rt, roleConfigGroupName: .rcgn, name: .rc.name, value: .rc.value } ),
            
            # Role config group configurations
            ( .items[] | { sn: .name, sdn: .displayName, st: .type, rcg: .roleConfigGroups[] } | 
              {sn: .sn, sdn: .sdn, st: .st, rcgn: .rcg.name, rcgdn: .rcg.displayName, rcgrt: .rcg.roleType, rcgb: .rcg.base, rcgc: (if (.rcg.config.items | length) > 0 then (.rcg.config.items[]) else null end) } | 
              {serviceName: .sn, serviceDisplayName: .sdn, serviceType: .st, roleConfigGroupName: .rcgn, roleConfigGroupDisplayName: .rcgdn, roleConfigGroupRoleType: .rcgrt, roleConfigGroupBase: .rcgb, name: (if .rcgc then .rcgc.name else null end), value: (if .rcgc then .rcgc.value else null end) } )
        ] | sort_by(.serviceDisplayName,.serviceType,.roleConfigGroupDisplayName,.roleConfigGroupRoleType,.name)' "$temp_file" > "$OUT_CLUSTER_SETTINGS"; then
            
            log_message "INFO" "Configuration data extracted successfully to: $OUT_CLUSTER_SETTINGS"
            
            # Display file size
            local file_size=$(du -h "$OUT_CLUSTER_SETTINGS" | cut -f1)
            log_message "INFO" "Output file size: $file_size"
        else
            log_message "ERROR" "Failed to process configuration data with jq"
            exit 1
        fi
    else
        log_message "ERROR" "Failed to retrieve configuration data from API"
        exit 1
    fi
...
cleanup() {
    ...
    log_message "INFO" "Output files available in: $OUTPUT_DIR"
}
...
main() {
    ...
    clear
    log_message "INFO" "Cloudera Manager Configuration Extractor"
    log_message "INFO" "====================================="
    ...
    # Load configuration and extract data
    load_cm_configuration
    ...
    if test_credentials; then
        ...
        prepare_output_directory
        extract_cluster_configuration
        log_message "INFO" "Configuration extraction completed successfully!"
    else
